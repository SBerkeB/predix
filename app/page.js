...
export default function Home() {
  const { predictions, loading, error, colors } = useApp();
  ...
  useEffect(() => {
    ...
    if (selectedCategory !== 'all') {
      filtered = filtered.filter(prediction => prediction.category.toLowerCase() === selectedCategory.toLowerCase());
    }
    ...
    if (searchTerm) {
      filtered = filtered.filter(prediction => 
        ...
        prediction.tags.some(tag => tag.toLowerCase().includes(searchTerm.toLowerCase()))
      );
    }

    setFilteredPredictions(filtered);
  }, [predictions, selectedCategory, searchTerm]);
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-8 max-w-2xl mx-auto mb-12">
              ...
              <div className="text-center">
                <div 
                  ...
                  style={{ color: colors.text.accent }}
                >
                  {predictions.reduce((total, prediction) => total + prediction.totalVotes, 0)}
                </div>
                <div style={{ color: colors.text.muted }}>
                  Total Votes
                </div>
